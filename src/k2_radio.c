...
char test_K2[40][15]={{0x55, 0x04, 0x01, 0x0B, 0x65},		// ->+ 0
					...
					{0x55, 0x04, 0x01, 0x02, 0x5C},         //13 A1 02 5C
					...
					//{0x55, 0x05, 0x01, 0x2A, 0x02, 0x87}, //32 ОБР-НП
					//{0x55, 0x05, 0x01, 0x2A, 0x03, 0x88}, //32 ОБР-ВИ
					{0x55, 0x05, 0x01, 0x2A, 0x06, 0x8B}, //32 ОБР-ТЕСТ
					{0x55, 0x05, 0x01, 0x22, 0x01, 0x7E}, //33 BLK_ONN
...
write_com27 (Ncom)
{
	...
	short D=0;
	int i;
	D=(short)(p->from_MO3.from41.D/1000);
	if (TS) D=0;//10000;
...
main(int argc, char *argv[]) {
	...
	p->to_MO3.to41.PrM_K2=p->to_MO3.to42.priem_K2=0;
 						...
 						case 5: if(buffer[i+2]==2)
								{
									switch(buffer[i+3])
									{
										...
										case 0x010: printf("  ПРМ - ");
													...
													if (buffer[i+4]&0x02) {printf(" СС");p->to_MO3.to42.priem_K2=p->to_MO3.to41.PrM_K2=1;}
																			else {p->to_MO3.to41.PrM_K2=p->to_MO3.to42.priem_K2=0;}
													...
													if (buffer[i+4]) p->to_MO3.to42.priem_K2=1; else p->to_MO3.to42.priem_K2=0;; 
 						...
 						case 6: if(buffer[i+2]==2)
								{
									switch(buffer[i+3])
									{
										...
										case 0x11 : printf(" Pтек - %d ",buffer[i+4]);
													if (buffer[i+4]>0)	{p->to_MO3.to42.priem_K2=1; p->to_MO3.to41.UR_sign_K2=buffer[i+4]*3;}
														else {p->to_MO3.to41.UR_sign_K2=0;p->to_MO3.to42.priem_K2=0;}
													break;
										case 0x73 : printf(" ПРМ1 - ");
													if (buffer[i+4]&0x01) {printf(" ПРС");}//p->to_MO3.to41.PrM_K2=1;}else p->to_MO3.to41.PrM_K2=0;
													//if (buffer[i+4]&0x02) printf(" СС");
													if (buffer[i+4]&0x02) {printf(" СС");p->to_MO3.to41.PrM_K2=1;}else p->to_MO3.to41.PrM_K2=0;
													if (buffer[i+4]&0x04) printf(" ЗС");
													if (buffer[i+4]&0x08) printf(" СБ");
													if (buffer[i+4]&0x10) {printf(" ИДК");p->to_MO3.to41.Pr_ZI_K2=1;}else p->to_MO3.to41.Pr_ZI_K2=0;
													if (buffer[i+4]&0x20) printf(" ИДД");
													...
													//printf("\n");
													if (buffer[i+4]||buffer[i+5]) p->to_MO3.to42.priem_K2=1; else p->to_MO3.to42.priem_K2=0;; 
													break;
									}
								}
								else printf(" Получен неизвестный пакет");								
...
/**/			case 9: if (comOK[5]>0) {comOK[5]=Ncount=0;N_COM+=4;break;}
			   ...
			   case 16: read_kvit();break;
 			   case 17: write_com(13);printf(")  Команда Реж-А");
						if (test_K2[13][3]==3)	printf("Г1 отправлена\n"); 
						else printf("1 отправлена\n");
						break;				 

			   case 18: read_kvit();break;
 			   case 19: write_com(26);printf(")  Команда К2-");
						if (test_K2[26][4]==1)	printf("ВКЛ отправлена\n"); 
						else printf("ВЫКЛ отправлена\n"); 
						break;
			   ...
			   case 23: //if (TS) //!!!!!!!!!!!!!!!!!!!!!!!!
						{	
							...
							//p->toPR1[3]=0xFC00;//31dBm
						    //p->toPR1[3]=0x0000;//OFF
							//p->toPR1[3]=0xBC00; //15dBm
						}
						N_COM++;break;
			   case 24:	N_COM++;break;
			   ...
			   case 35: read_kvit();break;
 			   case 36: write_com24(24);printf("  Начата посылка команды ДанИ.V.dV\n");
						...
						Tstart=p->from_MO3.from41.T_SS-10; //время старта за 10 сек до сеанса
						printf("\nT_SS=%d\n",p->from_MO3.from41.T_SS);
...
//----------------------------------------------------------------------------
			...
			//-----------------------------------
			   ...
			   case 41: read_kvit();break;
 			   ...
 			   case 43: /* (TS)	
						{
							if (Tcount>Tcount_com+100) {write_com(23);N_COM=37;}
							else if (comOK[24]>0) {comOK[24]++;if (comOK[24]>10) {comOK[24]=1;write_com24(24);}} 
						}
						else 
						{
							if (Tcount>Tcount_com+240) {write_com(23);N_COM=37;}
