...
char test_K2[40][15]={{0x55, 0x04, 0x01, 0x0B, 0x65},		// ->+ 0
					...
					//{0x55, 0x05, 0x01, 0x09, 0x02, 0x66},   //12 ДкР
					{0x55, 0x05, 0x01, 0x09, 0x02, 0x66},   //12 ДкK
					{0x55, 0x04, 0x01, 0x03, 0x5D},         //13 AG1 03 5D
					...
					//{0x55, 0x05, 0x01, 0x22, 0x01, 0x7E}, //33 BLOCK_ONN
					{0x55, 0x05, 0x01, 0x22, 0x0, 0x7D}, //33 BLOCK_OFF
					{0x55, 0x05, 0x01, 0x24, 0x06, 0x85}, //34 Выв. КД
					{0x55, 0x05, 0x01, 0x2A, 0x04, 0x89}}; //35 ОБР-СБ
...
write_com (Ncom)
{
	...
	//1m_sec=div(Tcount,10);
	//1strftime(b, 40 , "%T", localtime(&time_of_day));//D T
	//1printf("%s:%03d <-- (",b,m_sec.rem*100);			
	...
	while(1)
	{
		...
		{  //срабатывание таймера
			...
			switch (N_COM) //порядок выполнения команс в сеансе
			{
 			   ...
 			   case 9: write_com(12);printf(")  Команда ДК-К отправлена\n");break;
			   case 10: read_kvit();break;
 			   case 11: write_com(13);printf(")  Команда Реж-АГ1 отправлена\n");break;
			   case 12: read_kvit();break;
 			   case 13: write_com(26);printf(")  Команда К2-ВКЛ отправлена\n");break;
			   case 14: read_kvit();break;
 			   case 15: write_com(33);printf(")  Команда БЛК-ВЫКЛ отправлена\n");break;
			   case 16: read_kvit();break;

//----------------------------------------------------------------------------
//----------------------------------------------------------------------------
			   case 17: if (TM) {N_COM++;break;}
						...
						break;
			//-----------------------------------
 			   case 18: if (TS) {if (Tcount>Tcount_com+40) {write_com(32);printf("\n\nОБР ТЕСТ\n\n");}}
						else N_COM++;
						break;			   
			   case 19: write_com(34);N_COM++;break;
			   case 20: break;
			}
