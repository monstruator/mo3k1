...
//	start_time=clock();
    ...
    while(1) 
	{
		...
		if (pid==proxy) //обработчик таймера
		{
			...
			if ( (p->from_MO3.from41.num_com==2) && (TC10==1) ) 
			{
				fread(&log_shot, sizeof(log_shot),1,out_fp_log);
				printf("%02x:%02x:%02x \n", log_shot.CEB[2]>>8,log_shot.CEB[3]>>8,log_shot.CEB[3]&0x00ff);

				p->to_MO3.to41.GL_priem=log_shot.GL_priem;	
				p->to_MO3.to41.GL_CP=log_shot.GL_CP;			//	наличие сигнала "ср" на выходе Глории
				p->to_MO3.to41.UR_sign_K1=log_shot.UR_sign_K1;
				p->U.DPL_1=log_shot.DPL_1;


				if (num_data!=log_shot.cr_SEANCE)
				{
					//fread(&p->to_MO3.to41.DCP_K1, log_shot.sum_DCP*4,1,out_fp_data);
					//for(i=0;i<log_shot.sum_DCP;i++) 	
					fread(&p->to_MO3.to41.DCP_K1[0], 4,log_shot.sum_DCP,out_fp_data);
	
					p->to_MO3.to41.cr_SEANCE=log_shot.cr_SEANCE;
					p->to_MO3.to41.sum_DCP=log_shot.sum_DCP;


					for(i=0;i<10;i++) printf(" %08x",p->to_MO3.to41.DCP_K1[i]);printf("\n");
			...
			switch (TC10)
			{
				...
				case 9 : //раз в пол сек выполняем сервисные операции
						...
						//if (abs(p->from_MO3.from41.Fd*1000-Dopler1) > 2000) 
...
/*	log_shot.SUM_20=p->U.SUM_20;
	...
	//v 4.1-----------------------------------------------
	log_shot.P_FACT=p->to_MO3.to41.P_FACT;			//азимут, рад
	log_shot.beta_FACT=p->to_MO3.to41.beta_FACT;		//угол места, рад

	log_shot.UR_sign_K1=p->to_MO3.to41.UR_sign_K1;		//уровень сигнала по каналу К1 (0-100) -табло сигнала К1
	log_shot.UR_sign_K2=p->to_MO3.to41.UR_sign_K2;		//уровень сигнала по каналу К2 (0-100) -табло сигнала К2
	log_shot.GL_priem=p->to_MO3.to41.GL_priem;		//наличие сигнала "прием" на выходе Глории
									//(1-есть сигнал, 0- нет сигнала)
									//(табло Глория-прием)	
	log_shot.GL_CP=p->to_MO3.to41.GL_CP;			//	наличие сигнала "ср" на выходе Глории
									//(1-есть сигнал, 0- нет сигнала)
									//(табло Глория-ср)	
	log_shot.cr_SEANCE=p->to_MO3.to41.cr_SEANCE;		//порядковый номер массива данных цикла передачи 
									//0-USHRT_MAX
	log_shot.sum_DCP=p->to_MO3.to41.sum_DCP;		//количество принятых слов с массиве дцп
	//v 4.2-----------------------------------------------
	log_shot.Mispr=p->to_MO3.to42.Mispr;		//Маска исправности аппаратуры пр 3.1
	//iz 4.1----------------------------------------------
	log_shot.cr_com=p->from_MO3.from41.cr_com;	//порядковый номер последней команды, выданной из пр.4.1 в пр.3.1 
							//0-USHRT_MAX
	log_shot.num_com=p->from_MO3.from41.num_com;//номер последней выданной команды
							//команда 1 - подготовка к сеансу связи
							//команда 2 - начало сеанса связи
							//команда 3 - завершение  сеанса связи

							//параметры команды 1	
	log_shot.num_KS=p->from_MO3.from41.num_KS;	//номер канала связи 1 - нр-к1, 2 - пр-к2 
	log_shot.Nkey_SHAKR=p->from_MO3.from41.Nkey_SHAKR;	// номер ключа ШАКР 
	log_shot.P_ANT_1=p->from_MO3.from41.P_ANT_1;	//азимут антенны (в момент начала сеанса), рад
	log_shot.beta_1=p->from_MO3.from41.beta_1;		//угол места антенны (в момент начала сеанса), рад
	log_shot.Fd_1=p->from_MO3.from41.Fd_1;		//сдвиг частоты (в момент начала сеанса), Кгц 

	log_shot.T_SS=p->from_MO3.from41.T_SS;		//время начала СС сек в формате СЕВ
								//(время первого включения КПИ после входа КА в круг связи)

								//положение антенны и сдвиг частоты на каждом такте
	log_shot.P_ANT=p->from_MO3.from41.P_ANT;		//азимут антенны, рад
	log_shot.beta=p->from_MO3.from41.beta;		//угол места антенны, рад
	log_shot.Fd=p->from_MO3.from41.Fd;			//сдвиг частоты, Кгц 
*/
	printf("\n");
}