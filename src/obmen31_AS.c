  ...
  #define max_len_OUT    4096*8
	...
	obmen_MO3K_ASTRA_t   toAstra;
	obmen_ASTRA_MO3K_t fromAstra;
...
while(1)
  {
    ...
    memcpy(&rec4,&bufi[4],sizeof(obmen_ASTRA_MO3K_t)); 
	//выбор управляюще1 команды
	if (rec4.from42.cr_com!=cr_com42) 
	{
		printf(" New Command 4.2 = %d  (4.1) cr_com = %d\n",
	...
	if ((rec4.fromAK.cr_com!=cr_comAK)&&(rec4.fromAK.num_com!=0)) 
	{
		...
		switch(p->num_com)
		{
			case 300 : 	switch(p->from_MO3.fromAK.a_params[0])
						{
							case 0: if (p->from_MO3.fromAK.a_params[1]==1) {paramAKcom=1;p->M[2]=p->M[2]|0x0001;printf("AK\n");} else {printf("KA\n");paramAKcom=2;p->M[2]=p->M[2]&0xFFFE;}break;
							case 1:	if (p->from_MO3.fromAK.a_params[1]==1) {paramAKcom=3;p->M[2]=p->M[2]|0x0001;printf("AK\n");} else {printf("KA\n");paramAKcom=4;p->M[2]=p->M[2]&0xFFFE;}break;
						}
						break;
...
/////////////////////////////////////////////////////////////////////	
	...
	for(i1=0;i1<6;i1++)  toAstra.toNT.oCEB[i1]=p->CEB[i1];
	for(i1=0;i1<15;i1++) toAstra.toNT.oHK[i1]=p->Dout41[i1];
...
//////////////////////////////////////////////////////////////////////
  	len_OUT = sizeof(obmen_MO3K_ASTRA_t); //!!!!
	toAstra.to41=p->to_MO3.to41;
	toAstra.to42=p->to_MO3.to42;
	toAstra.toAK=p->to_MO3.toAK;
    memcpy(ip_out.out_buf,&toAstra,sizeof(obmen_MO3K_ASTRA_t)); ///!!!!!!
    if (len_OUT <= 1400) 
    {
         pack_buf[0] = 80 ;               
